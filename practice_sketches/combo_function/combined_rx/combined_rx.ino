  ...
  if (rf95.available())
  {
    ...
    {      
      ...
      if((char*)buf[0] == 'H') {
        ...
        strcpy(data, (uint8_t*)"Turned valve on."); // or use strcpy_s and input strlen("Turned valve on.") for the length it requests
        } else {
        data = new uint8_t[strlen("Turned valve off.")];
        digitalWrite(LED_PIN, LOW);
        digitalWrite(RELAY_PIN, LOW);
        strcpy(data, (uint8_t*)"Turned valve off."); // or use strcpy_s and input strlen("Turned valve off.") for the length it requests
        }
